...
- 商业计划书、执行摘要、项目简介
- 路演 PPT 内容、页面标题、项目亮点
- 调用 `codex-ppt` 生成高颜值商业计划书 PPT 或路演材料
- 园区、孵化器、创业社区申请材料
...
```text
调用商业计划书 skill 整理路演逻辑，再调用 codex-ppt 生成一份高颜值 16:9 商业计划书 PPT。
```
...
当目标不只是写出商业计划书，而是输出一份好看的 PPT 时，建议先用本 skill 整理内容逻辑，再调用 `codex-ppt` 做视觉化。

需要注意：本 skill 本身不是代码插件，不能在内部“自动执行” `codex-ppt`。它的作用是提供一套明确的协作流程，让 Codex 在同一次任务中先调用 `business-plan-template` 梳理商业计划书逻辑，再调用 `codex-ppt` 生成高颜值 16:9 视觉版 PPT。

推荐流程：

1. 用本 skill 明确项目定位、市场背景、产品方案、商业模式、团队能力、竞争优势、财务预测。
2. 将每页 PPT 压缩成一个核心观点、2-4 个支撑要点和一个视觉表达建议。
3. 调用 `codex-ppt` 生成统一风格的 16:9 视觉版商业计划书。
4. 自查是否覆盖团队能力、商业模式、竞争优势、市场空间、财务数据。

推荐这样向 Codex 发起请求：

```text
调用 business-plan-template skill，基于我的项目信息整理商业计划书路演逻辑，并调用 codex-ppt 生成一份高颜值 16:9 PPT。
```

或：

```text
用商业计划书 skill 先生成路演大纲，再用 codex-ppt 做视觉美化版 PPT。
```
